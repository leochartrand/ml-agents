...
class ACSettings(OnPolicyHyperparamSettings):
    beta: float = 5.0e-3
    lambd: float = 0.95
    num_epoch: int = attr.ib(default=1)  # AC does just one pass
...
class ACOptimizer(TorchOptimizer):
    def __init__(self, policy: TorchPolicy, trainer_settings: TrainerSettings):
        ...
        self.hyperparameters: ACSettings = cast(
            ACSettings, trainer_settings.hyperparameters
        )
    ...
    def update(self, batch: AgentBuffer, num_sequences: int) -> Dict[str, float]:
        ...
        """
        assert num_sequences == 1, "AC update requires a batch of size 1"
        ...
        memories = ModelUtils.list_to_tensor(
            batch[BufferKey.MEMORY]
        )
